...
static int enable_tx_csum = 1; 
static int enable_tso = 0;     
...
/**************************************************************************************************/
//Add by GanFan, to customize compability settings
module_param(enable_tx_csum, int, 0);
MODULE_PARM_DESC(enable_tx_csum, "Enable or disable TX checksum for compatibility of virual NIC.");

module_param(enable_tso, int, 0);
MODULE_PARM_DESC(enable_tso, "Enable or disable TCP Segmentation Offload (TSO) .");
/**************************************************************************************************/
...
//9.009 had have--by wangyf
//It's sad that VMKernel doesn't support line partner advertisement, so we have to backport rtl8125_gset_xmii to 9.005.06
...
//Multi-queue R/W is not supported by VMKernel
...
bool isMulticastType(__u8 mhead)
{
	struct __skbuff_mhead head;
	memcpy(&head, &mhead, sizeof(__u8));
	return head.pkt_type == PACKET_MULTICAST;
}

//PCI power management backport from 2.6.32
static void
...
{
	int pm_cap = pci_find_capability(dev, PCI_CAP_ID_PM);
	if (pm_cap == 0)
		return false;
	u16 value=0;
	int pm=0;
	...
	value >>= ffs(PCI_PM_CAP_PME_MASK) - 1; 
                 ...
                 const struct pci_device_id *ent)
{
		...
		/* Add by GanFan, to set net features according to parameters
		 * In ESXi,use 'esxcli system module parameters set (or list) ' command 
		 * e.g. esxcli system module parameter set -m r8125 -p "enable_tso=0"
		 */	
#if defined (__VMKLNX__)
			if (enable_tx_csum)
                dev->features |= NETIF_F_IP_CSUM | NETIF_F_IPV6_CSUM;
...
//A lot of backport here, rewrite this funtion //
#if !defined(__VMKLNX__)
...
//ESXi VMKernel hasn't global var system_state  //test2
#if !defined(__VMKLNX__)	
